...
// Manages all MAX groups functionality
//
...
function initializeGroupsModals() {
    ...
    $('#loadContactsBtn').on('click', async function () {
        ...
        try {
            ...
            statusText.html(
                '<i class="check icon"></i> Contacts loaded successfully! You can now select participants.'
            );
            ...
            statusText.html(
                '<i class="exclamation triangle icon"></i> Failed to load contacts. Please try again.'
            );
        } finally {
    ...
    $('#loadContactsForParticipantsBtn').on('click', async function () {
        ...
        try {
            ...
            statusDiv.removeClass('info').addClass('success');
            statusText.html(
                '<i class="check icon"></i> Contacts loaded successfully!'
            );
...
async function loadGroups() {
    try {
        ...
        if (response.code === 200 && response.data && response.data.Groups) {
            ...
            showError(
                'Failed to load groups: ' + (response.error || 'Unknown error')
            );
...
function createGroupListItem(group) {
    ...
    const admins = group.Participants
        ? group.Participants.filter((p) => p.IsAdmin && !p.IsSuperAdmin)
        : [];
    const superAdmins = group.Participants
        ? group.Participants.filter((p) => p.IsSuperAdmin)
        : [];
    ...
    const createdDate = group.GroupCreated
        ? new Date(group.GroupCreated).toLocaleDateString('en-US')
        : 'Unknown';
    ...
    if (isSuperAdmin) {
        userRoleBadge =
            '<span class="ui mini red label"><i class="shield icon"></i>Super Admin</span>';
    } else if (isAdmin) {
        userRoleBadge =
            '<span class="ui mini orange label"><i class="star icon"></i>Admin</span>';
    }
    ...
    if (group.IsAnnounce) {
        featureBadges +=
            '<span class="ui mini grey label"><i class="announcement icon"></i>Admins only</span>';
    }
    if (group.IsLocked) {
        featureBadges +=
            '<span class="ui mini grey label"><i class="lock icon"></i>Locked</span>';
    }
    ...
    if (group.IsJoinApprovalRequired) {
        featureBadges +=
            '<span class="ui mini grey label"><i class="checkmark icon"></i>Approval required</span>';
    }

    // Show linked parent for community groups
    let parentInfo = '';
    if (typeInfo.label === 'Community Group' && group.LinkedParentJID) {
        const parentGroup = groupsCache.Groups
            ? groupsCache.Groups.find((g) => g.JID === group.LinkedParentJID)
            : null;
        const parentName = parentGroup ? parentGroup.Name : 'Unknown Community';
        parentInfo = `<div class="meta"><i class="sitemap icon"></i>Part of: ${escapeHtml(
            parentName
        )}</div>`;
    }

    return `
        <div class="item group-list-item" data-group-id="${
            group.JID
        }" data-is-admin="${isAdmin}" data-is-super-admin="${isSuperAdmin}" data-group-type="${
        typeInfo.label
    }">
            <div class="ui grid">
                <div class="one wide column center aligned">
                    <i class="${typeInfo.icon} large ${
        typeInfo.color
    } icon"></i>
                </div>
                <div class="eleven wide column">
                    <div class="content">
                        <div class="header">
                            ...
                            <span class="ui mini ${
                                typeInfo.color
                            } basic label">${typeInfo.label}</span>
                            ${userRoleBadge}
                        </div>
                        ${parentInfo}
                        <div class="description group-description" data-full-text="${escapeHtml(
                            topic
                        )}">
                            ${escapeHtml(truncatedTopic)}
                            ${
                                topic.length > 100
                                    ? ' <a href="#" class="read-more-link" onclick="toggleDescription(this)">Read more</a>'
                                    : ''
                            }
                        </div>
                        <div class="meta">
                            ...
                            ${
                                admins.length > 0
                                    ? `<span><i class="star icon"></i>${admins.length} admins</span>`
                                    : ''
                            }
                            ${
                                superAdmins.length > 0
                                    ? `<span><i class="shield icon"></i>${superAdmins.length} super admins</span>`
                                    : ''
                            }
                ...
                <div class="four wide column right aligned">
                    <div class="ui small buttons">
                        <button class="ui primary button" onclick="viewGroupDetails('${
                            group.JID
                        }')">
...
function populateGroupDetailsModal(groupData) {
    ...
    $('#groupDetailsParticipants').text(
        (groupData.Participants || []).length + ' participants'
    );

    // Format creation date
    const createdDate = groupData.GroupCreated
        ? new Date(groupData.GroupCreated).toLocaleDateString('en-US', {
              year: 'numeric',
              month: 'long',
              day: 'numeric',
          })
    ...
    if (typeInfo.label === 'Community Group' && groupData.LinkedParentJID) {
        const parentGroup = groupsCache.Groups
            ? groupsCache.Groups.find(
                  (g) => g.JID === groupData.LinkedParentJID
              )
            : null;
        const parentName = parentGroup ? parentGroup.Name : 'Unknown Community';
    ...
    if (groupData.Participants && groupData.Participants.length > 0) {
        groupData.Participants.forEach((participant) => {
            ...
            const isCurrentUser =
                currentUserPhone &&
                participantPhone &&
                currentUserPhone === participantPhone;
                ...
                <div class="item participant-item">
                    <div class="content">
                        <div class="participant-info">
                            <div class="header">${escapeHtml(
                                participant.DisplayName || phone
                            )}</div>
                        ...
                        <div class="participant-actions">
                            <div class="participant-labels">
                                ...
                                ${
                                    isCurrentUser
                                        ? '<span class="ui mini blue label">You</span>'
                                        : ''
                                }
                            </div>
                            ${actionButtons}
        ...
        participantsList.append(
            '<div class="item">No participants found</div>'
        );
    }
}
...
function showCreateGroupModal() {
    ...
    dropdown
        .find('.menu')
        .html('<div class="item disabled">No contacts loaded</div>');

    // Show initial status message
    statusDiv.show().removeClass('success error').addClass('info');
    statusText.html(
        '<i class="info circle icon"></i> Click "Load Contacts" to populate the participants list.'
    );
...
function populateParticipantsDropdown() {
    ...
    if (
        contactsCache &&
        typeof contactsCache === 'object' &&
        Object.keys(contactsCache).length > 0
    ) {
        Object.entries(contactsCache).forEach(([jid, contact]) => {
            const phone = contact.Phone;
            const name = contact.FullName || contact.PushName || phone;
            dropdown.append(
                `<div class="item" data-value="${phone}">${escapeHtml(
                    name
                )} (+${phone})</div>`
            );
        });
        ...
        dropdown.append(
            '<div class="item disabled">No contacts loaded. Click "Load Contacts" to populate the list.</div>'
        );
...
async function createGroup() {
    try {
        ...
        if (response.success || response.code === 200) {
            ...
            showError(
                'Failed to create group: ' + (response.error || 'Unknown error')
            );
        }
    } catch (error) {
...
async function joinGroup() {
    try {
        ...
        if (response.success) {
            ...
            showError(
                'Failed to join group: ' + (response.error || 'Unknown error')
            );
        }
    } catch (error) {
...
function toggleDescription(element) {
    ...
    if (element.textContent.trim() === 'Read more') {
        descriptionDiv.innerHTML =
            escapeHtml(fullText) +
            ' <a href="#" class="read-more-link" onclick="toggleDescription(this)">Read less</a>';
    } else {
...
// Enhanced function to get user info and set user ID (MAX uses numeric user IDs)
async function getUserInfoAndSetJID() {
    try {
        // Try to get user info from status API to get the user ID
        const token = getLocalStorageItem('token');
        const myHeaders = new Headers();
        myHeaders.append('token', token);
        myHeaders.append('Content-Type', 'application/json');
        ...
        if (data.code === 200 && data.data && data.data.max_user_id) {
            const userID = data.data.max_user_id;
            localStorage.setItem('currentUserJID', userID); // Keep key name for compatibility
            window.currentUserJID = userID;
            //console.log('User ID set from status:', userID);
            return userID;
        }
    } catch (error) {
...
function showEditGroupInfoModal() {
    ...
    const groupData = groupsCache.Groups
        ? groupsCache.Groups.find((g) => g.JID === currentGroupId)
        : null;
    if (groupData) {
...
function showGroupSettingsModal() {
    ...
    if (groupData) {
        ...
        $('#announceOnlyToggle').checkbox(
            groupData.IsAnnounce ? 'check' : 'uncheck'
        );
...
function showGroupInviteLinkModal() {
    ...
    getGroupInviteLink(currentGroupId)
        .then((response) => {
...
function populateCurrentParticipantsList() {
    ...
    if (groupData && groupData.Participants) {
        const currentUserJID = getCurrentUserJID();
        const currentUserIsSuperAdmin = checkIfUserIsSuperAdmin(
            groupData,
            currentUserJID
        );
        const currentUserIsAdmin = checkIfUserIsAdmin(
            groupData,
            currentUserJID
        );
        ...
        groupData.Participants.forEach((participant) => {
            ...
            if (
                !isCurrentUser &&
                (currentUserIsAdmin || currentUserIsSuperAdmin)
            ) {
                const buttons = [];
...
function populateAddParticipantsDropdown() {
    ...
    if (
        ...
        const currentParticipants =
            groupData && groupData.Participants
        ...
        $('#addParticipantsDropdown .default.text').text(
        ...
        if (!nameResponse.success) {
            throw new Error(
                'Failed to update group name: ' +
                    (nameResponse.error || 'Unknown error')
            );
        }

        // Update group description if provided
        if (groupDescription) {
            const topicResponse = await updateGroupTopic(
                currentGroupId,
                groupDescription
            );
            if (!topicResponse.success) {
                throw new Error(
                    'Failed to update group description: ' +
                        (topicResponse.error || 'Unknown error')
                );
            }
        }
...
async function saveGroupSettings() {
    ...
    try {
        ...
        const disappearingTimer = $('#disappearingTimerSelect').dropdown(
            'get value'
        );

        // Update announce setting
        const announceResponse = await updateGroupAnnounce(
            currentGroupId,
            announceOnly
        );
        ...
        const ephemeralResponse = await updateGroupEphemeral(
            currentGroupId,
            disappearingTimer
        );
...
async function addParticipants() {
    ...
    const selectedParticipants = $('#addParticipantsDropdown').dropdown(
        'get value'
    );
...
async function promoteParticipant(participantJID) {
    ...
    if (
        !confirm('Are you sure you want to promote this participant to admin?')
    ) {
        return;
    }
...
async function demoteParticipant(participantJID) {
    ...
    if (
        !confirm(
            'Are you sure you want to demote this admin to regular participant?'
        )
    ) {
        return;
    }
...
async function copyInviteLink() {
    ...
    if (
        !inviteLink ||
        inviteLink === 'Loading...' ||
        inviteLink.includes('Failed') ||
        inviteLink.includes('Error')
    ) {
...
async function resetInviteLink() {
    ...
    if (
        !confirm(
            'Are you sure you want to reset the invite link? This will invalidate the current link.'
        )
    ) {
        return;
    }
...
async function uploadGroupPhoto(file) {
    ...
    if (
        ...
        showError(
            'Please select a supported image file (JPEG, PNG, GIF, WebP)'
        );
        return;
    }
    ...
    try {
        ...
        if (uploadButton) {
            ...
            uploadButton.innerHTML =
                '<i class="spinner loading icon"></i> Processing...';
        }
        ...
        console.log('Processed image stats:', {
            ...
            estimatedSizeKB: Math.round(
                (processedImage.base64.length * 0.75) / 1024
            ),
            firstChars: processedImage.base64.substring(0, 20),
            lastChars: processedImage.base64.substring(
                processedImage.base64.length - 20
            ),
        });
        ...
        if (uploadButton) {
            uploadButton.innerHTML =
                '<i class="spinner loading icon"></i> Uploading...';
        }
        ...
        if (response.success) {
            ...
            if (errorMsg.includes('decode') || errorMsg.includes('base64')) {
                throw new Error(
                    'Image encoding error. Please try a different image file.'
                );
            } else if (
                ...
                throw new Error(
                    'Image format error. Please ensure the image is in a supported format (JPEG, PNG, GIF, WebP).'
                );
        ...
        if (error.message.includes('Network error')) {
            ...
            errorMessage =
                'Failed to process image. Please try a different image file.';
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = function () {
            try {
                ...
                if (
                    !dataUrl ||
                    !dataUrl.startsWith(`data:${outputFormat};base64,`)
                ) {
                    throw new Error(
                        `Failed to convert image to ${outputFormat} format`
                    );
                }
                ...
                try {
                    ...
                    throw new Error(
                        'Generated base64 string is invalid: ' + e.message
                    );
                }
        ...
        img.onerror = function (error) {
            ...
            reject(
                new Error(
                    'Failed to load image file. Please ensure it is a valid image.'
                )
            );
        };
...
async function getGroupInviteLink(groupJID, reset = false) {
    ...
    const response = await fetch(
        baseUrl + '/group/invitelink?' + params.toString(),
        {
            method: 'GET',
            headers: myHeaders,
        }
    ...
    try {
        ...
        if (response.status === 200 && data.code === 200) {
            ...
            if (data.error) {
                ...
                errorMessage =
                    'Bad request. Please check the image format and try again.';
            } else {
