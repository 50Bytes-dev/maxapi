...
async function addInstance(data) {
    ...
    const s3Config = {
        ...
        mediaDelivery: s3Enabled
            ? data.s3_media_delivery || 'base64'
            : 'base64',
...
function handleRegularLogin(token, notifications = false) {
    ...
    statusRequest().then((status) => {
        if (status && status.success == true && status.data) {
            ...
            // Save current user ID for groups functionality
            if (status.data.max_user_id) {
                setLocalStorageItem(
                    'currentUserID',
                    status.data.max_user_id,
                    6
                );
                window.currentUserID = status.data.max_user_id;
            }
...
function updateUser() {
    ...
    status().then((result) => {
        if (result && result.success == true && result.data) {
            // Save current user ID for groups functionality
            if (result.data.max_user_id) {
                setLocalStorageItem(
                    'currentUserID',
                    result.data.max_user_id,
                    6
                );
                window.currentUserID = result.data.max_user_id;
            }
...
function doUserAvatar() {
    ...
    if (phone) {
        // MAX uses plain phone numbers, no JID suffix needed
        userAvatar(phone)
            .then((data) => {
                document
                    .getElementById('userAvatarContainer')
                ...
                if (data.success && data.data && data.data.url) {
                    const userAvatarDiv = document.getElementById(
                        'userAvatarContainer'
                    );
...
async function connect(token = '') {
    ...
    // Already authenticated - proceed with connect
    ...
    data = await res.json();
    updateInterval = 1000; // Decrease interval to react quicker to status changes
    return data;
}
...
function populateInstances(instances) {
    ...
    // Handle null/undefined instances
    ...
    instances.forEach((instance) => {
      ...
      <tr>
        ...
        <td><i class="${
            instance.authenticated ? 'check green' : 'times red'
        } icon"></i> <span class="status ${instance.authenticated}">${
            instance.authenticated ? 'Yes' : 'No'
        }</span></td>
        <td>
  ...
  `;
      ...
      <div class="ui fluid card hidden no-hover" id="instance-card-${
          ...
          <div class="content">
              <div class="ui ${
                  instance.authenticated ? 'one' : 'two'
                  ...
                  <div class="column">
                      <div class="header" style="font-size: 1.3em; margin-bottom: 0.5rem;">
                          ...
                          <div class="ui labels" style="margin-top: 0.5em;">
                              ...
                              <div class="ui ${
                                  instance.authenticated ? 'green' : 'red'
                              } horizontal label">
                                  <i class="${
                                      instance.authenticated ? 'check' : 'times'
                                  } icon"></i>
                                  ${
                      ...
                      <div class="ui list">
                          ...
                          <div class="item">
                              <div class="header">MAX User ID</div>
                              <div class="content">${
                                  instance.max_user_id || 'Not available'
                              }</div>
                          </div>
                          <div class="item">
                  ...
                  <div class="column" style="display: flex; flex-direction: column; justify-content: center; align-items: center;">
                      <div class="ui segment" style="width: 100%; max-width: 250px; padding: 20px; text-align: center;">
                        <div class="ui icon header" style="text-align: center;">
                            <i class="plug icon" style="font-size: 3em;"></i>
                            <div class="sub header">Not Connected</div>
                        </div>
                        <p style="margin-top: 15px;">Click "Connect" to start the session.</p>
                      </div>
                    </div>
                    `
                          : `
                    <!--one column when connected-->
                    `
                  }
                </div>
            </div>
...
async function loadS3Config() {
    ...
    if (currentInstanceData && currentInstanceData.s3_config) {
        ...
        const hasConfig =
            s3Config.enabled || s3Config.endpoint || s3Config.bucket;
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.code === 200 && data.data) {
                ...
                $('#s3ForcePathStyle').prop(
                    'checked',
                    data.data.path_style || false
                );
...
async function saveHistoryConfig() {
    ...
    try {
        ...
        if (data.success) {
            ...
            showError(
                'Failed to save history configuration: ' +
                    (data.error || 'Unknown error')
            );
        }
    } catch (error) {
